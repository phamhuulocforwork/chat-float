...
interface Store {
  ...
  syncValue: (colorString: string) => void;
  notify: () => void;
}
...
function ColorPicker(props: ColorPickerProps) {
  ...
  const store = React.useMemo<Store>(() => {
    return {
      ...
      syncValue: (colorString: string) => {
        const parsed = parseColorString(colorString);
        if (!parsed) return;

        stateRef.current.color = parsed;
        stateRef.current.hsv = rgbToHsv(parsed);
        store.notify();
      },
      notify: () => {
...
function ColorPickerArea(props: DivProps) {
  ...
  return (
    <AreaPrimitive
      ...
      {...areaProps}
      className={cn(
        "relative h-30 w-full cursor-crosshair touch-none rounded-sm border",
...
function ColorPickerSwatch(props: DivProps) {
  ...
  return (
    <SwatchPrimitive
      ...
      {...swatchProps}
      className={cn(
        "box-border h-4 w-7 rounded-sm border border-input shadow-sm cursor-pointer ring-1",
